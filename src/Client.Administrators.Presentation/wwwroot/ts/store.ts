...
$(document).on("keypress", '#detail-address', (event: JQuery.KeyPressEvent) => {
    const key = event.key;
    if (!/^[a-zA-Z0-9\-|]$/.test(key)) {
        event.preventDefault();
    }
...
$(document).on("input", '#detail-address', function (this: HTMLInputElement) {
    this.value = this.value.replace(/[^a-zA-Z0-9\-|]/g, '');
});

function proceedCreateStoreForm(): boolean {
    ...
    const addressInputEl = document.querySelector<HTMLInputElement>('#detail-address');
    ...
    if (isProvinceSelected && isDistrictSelected && isWardSelected && addressInput !== "") {
        ...
        if (ward.split(/\s+/).length === 1) ward = "Phường " + ward;
        if (district.split(/\s+/).length === 1) district = "Quận " + district;

        return $(".create-store-form").valid();
    } else {
        $('.address-validation-message').show();

        if (!isProvinceSelected) $('#province-btn').css('border', '1px solid #e74c3c');
        if (!isDistrictSelected) $('#district-btn').css('border', '1px solid #e74c3c');
        if (!isWardSelected) $('#ward-btn').css('border', '1px solid #e74c3c');
        if (addressInput === "") $('#detail-address').css('border', '1px solid #e74c3c');
...
function getProvinces(): void {
    $.ajax({
        ...
        success: function (response: {
            ...
            body: { code: string, name: string, codeName: string }[]
        }) {
            ...
            dropdown.on('click', '.create-store-province-item', function (this: HTMLElement) {
...
function getDistricts(province: string): void {
    $.ajax({
        url: `${gatewayUrl}/districts?province=${province}`,
        type: 'GET',
        success: function (response: { isSuccessful: boolean, body: { code: string, name: string }[] }) {
            if (!response || !response.isSuccessful || !Array.isArray(response.body)) {
                console.error("Invalid API response format:", response);
                return;
            }
            ...
            dropdown.on('click', '.create-store-district-item', function (this: HTMLElement) {
...
function getWards(district: string): void {
    $.ajax({
        ...
        success: function (response: { isSuccessful: boolean, body: { code: string, name: string }[] }) {
            ...
            dropdown.on('click', '.create-store-ward-item', function (this: HTMLElement) {
                let button = $('#ward-btn');
                const text = this.textContent || '';
...
function getSelectedText(buttonId: string): string {
    const btn = document.getElementById(buttonId);
    return btn?.textContent?.trim() || '';
}

function isValidSelection(buttonId: string, defaultText: string): boolean {
